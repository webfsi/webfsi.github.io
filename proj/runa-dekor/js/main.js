jQuery(function($){
  ...
  $('.tabs-slider').on('afterChange', function(event, slick, currentSlide){
    $('.box').hide();
    $('.box[data-id=' + (currentSlide + 1) + ']').show();
  });
  ...
  // add-open-class
  $('.filter-st-item__cap_open').click(function(){
   if($(this).parent().is('.filter-st-item_opened')){
    $(this).parent().removeClass('filter-st-item_opened');
   }else{
    $(this).parent().addClass('filter-st-item_opened');
   }
  });
  ...
  jcf.setOptions('File', {
    ...
    placeholderText: 'Размер файла не должен превышать 2 мв'
  });
  ...
  $(window).scroll(function() {
    if ($(this).scrollTop() > $('.banner-boxes').height() + $('.header').height()){ 
